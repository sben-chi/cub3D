...
/*   Updated: 2023/02/11 21:34:32 by irhesri          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	*get_intersection_x(t_data *data, double teta, short *sym)
{
	size_t	px;
	double	dx;
	double	dy;
	...
	px = floor(data->p[0] / TILE);
	dx = fabs((floor(data->p[0] / TILE + (sym[0] == 1)) * TILE - data->p[0]));
	while (1)
	{
		px += sym[0];
		dy = fabs(dx * tan(teta));

		info[0] = data->p[0] + sym[0] * dx;
		info[1] = data->p[1] + sym[1] * dy;
		if ((info[1] >= (data->lines * TILE)) || (info[1] <= 0) || (info[0] >= ((data->max - 1) * TILE)) || (info[0] <= 0))
		{
			free(info);
			return (NULL);
		}
		if (data->map_arr[(int)(info[1] / TILE)][(int)(info[1] / TILE)])
		{
			info[0] -= (sym[0] == 1);
			info[1] -= (sym[1] == 1);
...
double	*get_intersection(t_data *data, double teta, short *sym, bool x_y)
{
	...
	if ((!x_y) && is_equale(teta, PI_2) || is_equale(teta, _3PI_2))
		return (NULL);
	info = my_calloc(4, sizeof(double));
	p[x_y] = floor(data->p[1] / TILE);
	d[x_y] = fabs((floor(data->p[1] / TILE + (sym[1] == 1)) * TILE - data->p[1]));
	while (1)
	{
		p[x_y] += sym[x_y];
		if (x_y)
...
void	get_dest(t_data *data, double teta, long long k)
{
	...
	ptr_x = get_intersection_x(data, teta, sym);
	ptr_y = get_intersection_y(data, teta, sym);
