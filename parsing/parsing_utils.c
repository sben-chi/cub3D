...
void	get_color(t_data *data, char *element, int k, int len)
{
	...
	while (++i < len)
	{
		...
		if (cl[j] > 255)
			put_error("Error: invalid nb of color1\n"));
		if (element[i] == ',' && j < 2)
			j++;
		else if (element[i] != '\n')
			put_error("Error: invalid nb of color2\n"));
	}
	if (cl[0] < 0 || cl[1] < 0 || cl[2] < 0)
		put_error("Error: invalid nb of color: {xxx,xxx,xxx}\n"));
